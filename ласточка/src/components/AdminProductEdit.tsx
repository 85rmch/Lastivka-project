...
import { Loader2, X, Trash2, CheckCircle2, AlertCircle, Eye, EyeOff } from 'lucide-react';
...
export default function AdminProductEdit({ product, onClose, onUpdate, lang = 'ua' }: { product: Product; onClose: () => void; onUpdate: () => void; lang?: 'ru' | 'ua' }) {
  ...
  const t = {
    ru: {
      ...
      ok: 'ОК',
      hideProductLabel: 'Скрыть карточку товара (не показывать в магазине)',
      hideProductDesc: 'Товар сохранится в базе данных, но не будет отображаться в каталоге для покупателей. Вы сможете включить его обратно, когда он снова появится в наличии.',
      statusHidden: 'Скрыт из магазина',
      statusVisible: 'Отображается в магазине'
    },
    ua: {
      ...
      ok: 'ОК',
      hideProductLabel: 'Сховати картку товару (не показувати в магазині)',
      hideProductDesc: 'Товар збережеться в базі даних, але не відображатиметься в каталозі для покупців. Ви зможете знову увімкнути його, коли він знову з\'явиться в наявності.',
      statusHidden: 'Схований з магазину',
      statusVisible: 'Відображається в магазині'
    }
  }[lang];
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/85 backdrop-blur-sm p-2 sm:p-4 overflow-y-auto">
      <div className="w-full max-w-lg bg-[#0d0d0d] border border-white/10 rounded-2xl shadow-2xl flex flex-col max-h-[92vh] sm:max-h-[88vh] my-auto overflow-hidden relative">
        
        {/* Header */}
        <div className="flex items-center justify-between p-4 sm:p-5 border-b border-white/10 shrink-0 bg-[#0d0d0d] z-10">
          <h3 className="text-white font-semibold text-sm sm:text-base truncate pr-2">{t.editTitle} {product.name}</h3>
          <button onClick={onClose} className="text-[#a19992] hover:text-white cursor-pointer p-1 rounded-lg hover:bg-white/10 transition-colors shrink-0"><X className="w-5 h-5" /></button>
        </div>
        ...
        <div className="p-4 sm:p-6 space-y-4 overflow-y-auto flex-1">
          ...
          {/* Hide / Show Product Card Toggle */}
          <div className={`p-3.5 rounded-xl border transition-all ${formData.isHidden ? 'bg-amber-500/10 border-amber-500/30' : 'bg-[#161616] border-white/10'}`}>
            <label className="flex items-start justify-between gap-3 cursor-pointer select-none">
              <div className="flex items-start gap-3">
                <div className={`p-2 rounded-lg mt-0.5 shrink-0 ${formData.isHidden ? 'bg-amber-500/20 text-amber-400' : 'bg-emerald-500/20 text-emerald-400'}`}>
                  {formData.isHidden ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                </div>
                <div>
                  <span className="text-white text-xs font-bold block">{t.hideProductLabel}</span>
                  <span className="text-gray-400 text-[11px] leading-tight block mt-0.5">{t.hideProductDesc}</span>
                  <span className={`inline-block mt-2 text-[10px] font-semibold px-2 py-0.5 rounded ${formData.isHidden ? 'bg-amber-500/20 text-amber-300 border border-amber-500/30' : 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/30'}`}>
                    {formData.isHidden ? t.statusHidden : t.statusVisible}
                  </span>
                </div>
              </div>
              <input 
                ...
                className="w-5 h-5 accent-[#d4af37] cursor-pointer mt-1 rounded shrink-0" 
              />
            </label>
          </div>
                      ...
                      {formData.photo.map((url, idx) => (
                          <div key={idx} className="relative group aspect-square">
                              <img 
                                  ...
                                  className="w-full h-full object-cover rounded-md border border-white/10 bg-[#111]" 
                              />
                              <button 
                                type="button" 
                                onClick={() => setFormData(prev => ({...prev, photo: (prev.photo as string[]).filter((_, i) => i !== idx)}))} 
                                className="absolute -top-1.5 -right-1.5 bg-red-500 hover:bg-red-600 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs font-bold shadow-md opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity cursor-pointer"
                                title="Удалить фото"
                              >
                                ×
                              </button>
        ...
        {/* Fixed Footer with Save/Delete buttons */}
        <div className="p-4 border-t border-white/10 shrink-0 bg-[#0d0d0d] flex gap-2 z-10">
          <button 
          ...
          <button 
             onClick={handleSave} 
             disabled={saving || uploading || deleting}
            className="flex-1 p-3 bg-[#d4af37] hover:bg-[#c5a028] text-black font-bold rounded-lg text-sm cursor-pointer flex items-center justify-center transition-colors shadow-md"
          >
